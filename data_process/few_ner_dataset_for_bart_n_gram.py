...
class FewShotNERDataset(data.Dataset):
    ...
    def __init__(self, filepath, tokenizer, N, K, Q, max_length, Ptuing=True, ignore_label_id=-1, n_gram=7, encoder_name='bart',args=None):
    ...
    def _get_entity_list (self,tokens,labels):
        ...
        for i in range(len(tokens)):
            for j in range(0, self.n_gram):
                if(i+j < len(tokens)):
                    span_label = labels[i]
                    for k in range(i+1, i+j+1):
                        if(span_label != labels[k]):
                            span_label = 0
                            break
                    entity_label_list.append(span_label)
                    entity_list.append([i,i+j])
    ...
    def __contact_question__for__bart(self, target_classes, raw_tokens, labels):
        ...
        assert (len(entity_list)==len(entity_label_list))
