...
from .configuration_bart import LightBartConfig
...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        #fewner part
        #query_states (num_head,seq_len,head_dim)
        key_states=torch.cat((key_states,self.prompt_k_matrix),dim=1)
        ...
        """ if attention_mask is not None:
            if attention_mask.size() != (bsz, 1, src_len, src_len):
                raise ValueError(
                    f"Attention mask should be of size {(bsz, 1,src_len, src_len)}, but is {attention_mask.size()}"
                )
        ...
        value_states=torch.cat((value_states,self.prompt_v_matrix),dim=1)
